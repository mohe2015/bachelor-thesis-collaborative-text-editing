...
<investigating-ot-algorithms>
In their review of the Fugue paper, shows that the claims in the Fugue paper #cite(<2023-weidner-minimizing-interleaving>) about ot being interleaving are not correct #cite(<2023-sun-critical-examination-fugue-ot>);#cite(<2023-sun-critical-examination-fugue-ot-1>);#cite(<2023-sun-critical-examination-fugue-ot-2>);#cite(<2023-sun-critical-examination-fugue-ot-3>);.
First, they show that mistakes were made in the Fugue paper when applying the ot algorithms which render their results regarding ot invalid #cite(<2023-sun-critical-examination-fugue-ot-1>);#cite(<2023-sun-critical-examination-fugue-ot-2>);.
They also show that interleaving has been examined and documented before and can be solved in ot, usually by having operations based on strings and not single characters, but this is also possible when operating on single characters #cite(<2023-sun-critical-examination-fugue-ot-2>);. Therefore, investigating OT algorithms, especially in a #emph[non-realtime] setting could be interesting.
...
While in some cases the full editing history needs to be kept to be able to attribute all changes, in other cases it can be reduced as much as possible without causing causality problems. The approach of the antimatter#footnote[#link("https://web.archive.org/web/20240623153539/https://braid.org/antimatter");] algorithm is to combine operations that have been seen by the same group of peers by tracking acknowledgements. In case peers go offline but come online at some point later it can potentially still combine operations.

Our current performance measurements only test non-concurrent actions. It may be beneficial to either find or create some real-world editing trace with concurrent actions or generate some artificial trace like in YATA #cite(<2016-yata-yjs>)[Section 6.1];.
