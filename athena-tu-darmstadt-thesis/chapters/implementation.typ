...
In practice, trees usually contain many deep right descendants because of consecutive character insertions #cite(<2023-weidner-minimizing-interleaving>)[Figure 5];, so this is a case that should be heavily optimized.
...
To properly use text editing algorithms an editor is required, so we implement an interface to ProseMirror#footnote[#link("https://prosemirror.net/");] and transpile Scala to JavaScript using Scala.js#footnote[#link("https://www.scala-js.org/");] to be able to use our implementation on the web.

By default, ProseMirror creates newlines using `<br/>`~tags and paragraphs using `<p>`~tags. This makes it complicated to convert between the ProseMirror document offset and the text offset. Therefore, we configured ProseMirror to only support plaintext and use `\n` for newlines and configured the browser to render `\n` as newlines \(which does not work by default) as shown in .

We also implemented a demo using WebRTC#footnote[#link("https://webrtc.org/");] to collaboratively edit a text. It keeps the full history on all connected peers, so it is not possible to permanently delete anything. This is the reason for not implementing persistence, see .

== Synchronization of Changes
<sec:synchronization>
The changes are synchronized using causal broadcast as in the Fugue paper #cite(<2023-weidner-minimizing-interleaving>);. The events are ordered using vector clocks #cite(<1988-mattern-vector-clock>);#cite(<1988-fidge-vector-clock>);. Only change synchronization updates the vector clock. Therefore, the clock does not need to be updated while working offline, and the changes can be sent in one batch which is more efficient. Instead of creating a message per character insertion or deletion, consecutive deletions and insertions that have the same causality are combined to optimize memory usage.

== Testing Using Property Tests
<sec:property-tests>
Property tests are a core part of testing rdts as the existence of numerous edge cases make unit testing infeasible. The tests run both on the internal data structure, with an interface for inserting and deleting characters at indices, and on the local web application as a Playwright#footnote[#link("https://playwright.dev/java/");] test.

The property tests run using ScalaCheck#footnote[#link("https://scalacheck.org/");] and specifically its stateful testing support#footnote[#link("https://github.com/typelevel/scalacheck/blob/main/doc/UserGuide.md#stateful-testing");] using `Commands`#footnote[<footnote:commands>#link("https://github.com/typelevel/scalacheck/blob/main/core/shared/src/main/scala/org/scalacheck/commands/Commands.scala");];. ScalaCheck `Commands` store a system under test and a state that is compared to the system under test. Possible actions are defined by implementing the `Command` trait. The trait has several methods for pre conditions, post conditions, running the action and calculating the next state. ScalaCheck generates `Command`s and their contents using Generators, e.g. `Gen.chooseNum(0, Int.MaxValue)` which are then run by ScalaCheck against the system under test and if failures occur it tries to simplify the failure case.

Our property tests randomly create replicas, synchronize replicas, insert text at a replica or delete text at a replica. Then, they check whether replicas have the same text after they synchronized. Unfortunately it is not easily possible to check #emph[what] the expected text would be as that would need more or less a reimplementation of the synchronization logic, see . We also have property tests that check that local operations match the same operations on a `String`.
...
While working on our implementation, we found that the algorithmic description #cite(<2023-weidner-minimizing-interleaving>)[Algorithm~1] is, for the most part, satisfactory. However, it contains one large issue. While the Fugue paper includes the conversion from character offsets to their internal representation, it misses the reverse direction #cite(<2023-weidner-minimizing-interleaving>)[Algorithm~1];. Received operations also need to be converted to the index to update the local text editor. Therefore, we extended the algorithmic description with that. This is not just relevant for implementation but also for optimization, which we address in the next chapter. It means that further functionality is required, that can map a node ID to the position in the tree traversal of visible nodes, which is the visible text. The remaining issues were only minor or instances of suboptimal specification.

First, the ID type #cite(<2023-weidner-minimizing-interleaving>)[Algorithm~1] can always be `null`. As this can only be the case for the root node, we moved this case to the places where the root node could potentially be used. There are some places where this could #emph[not] be the case, e.g. remote insertions can not send the root node as the root node is always locally created.

Second, in line 10 of the description #cite(<2023-weidner-minimizing-interleaving>)[Algorithm~1];, root is initialized with a value that is invalid according to their specification because the side can only be `L` or `R` but never `null` according to the types. Our implementation arbitrarily chooses the root node to be on the right side to simplify checks at other places in the code. An alternative would be to use an enumeration for the node and not have an ID, value, side and parent for the root node at all.

Third, each node does not necessarily need to store the ID of its parent and children #cite(<2023-weidner-minimizing-interleaving>)[Algorithm~1];. It could also store a reference directly to them.

Lastly, the node after `leftOrigin` in line 24 #cite(<2023-weidner-minimizing-interleaving>)[Algorithm~1] can be retrieved as the leftmost descendant of the first right child of the `leftOrigin`. The leftmost descendant is the node that is reached by repeatedly descending into the leftmost child until there are no left children. This is logical as the next node must be in the right subtree and there the first node is the leftmost node. Depending on the implementation that may be faster or easier.
